...
$regex = [regex]'(?s)<div class="card__icon"[^>]*>.*?</div>\s*<h3 class="card__title">(.*?)</h3>'

$newContent = $regex.Replace($content, {
    param($match)
    $title = $match.Groups[1].Value
    $icon = $icons[$title]
    if (-not $icon) { $icon = $icons["Оздоровительный массаж"] }
    return "<div class=`"card__icon`" aria-hidden=`"true`">`n                $icon`n              </div>`n              <h3 class=`"card__title`">$title</h3>"
})

[IO.File]::WriteAllText("index.html", $newContent, [Text.Encoding]::UTF8)
