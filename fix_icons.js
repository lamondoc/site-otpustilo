...
content = content.replace(/<div class="card__icon"[^>]*>[\s\S]*?<\/div>\s*<h3 class="card__title">(.*?)<\/h3>/g, (match, title) => {
    const icon = icons[title] || icons['Оздоровительный массаж'];
    return `<div class="card__icon" aria-hidden="true">\n                ${icon}\n              </div>\n              <h3 class="card__title">${title}</h3>`;
});
